...
<div class="col-12 col-md-6">
    ...
    <select class="form-select" name="attendee_id">
        ...
        @foreach ($agents as $attendee)
            <option value="{{ $attendee->uuid }}" {{ old('attendee_id', $model?->attendees()?->id)==$attendee->id ? 'selected' : '' }}>{{ $attendee->name }} ({{ $attendee->email }})</option>
        @endforeach
    </select>
...
<div class="col-12 col-md-6">
    ...
    <select name="status_id" id="status_id" class="select2 form-select status_id">
        ...
        @foreach($statuses as $status)
            <option value="{{$status?->uuid}}" {{ $lead?->lastStatusLog?->status_id==$status->id ? 'selected' : '' }}>{{ ucfirst($status->name ?? '-') }}</option>
        @endforeach
    </select>
