...
<div class="row g-3 mb-4">
    ...
    <div class="col-12 col-md-6">
        ...
        <select id="type" name="type" class="form-select">
            ...
            @foreach (campaignTypes() as $type)
                <option value="{{ $type }}" {{ $model->type==$type?'selected':'' }}>{{ ucfirst($type) }}</option>
            @endforeach
        </select>
        ...
        <select id="user_id" name="user_ids[]" class="form-select" multiple>
            @foreach ($agents as $agent)
                <option value="{{ $agent->uuid }}" 
                    {{ collect(old('user_ids', $model->agents->pluck('uuid')))->contains($agent->uuid) ? 'selected' : '' }}>
    ...
    <div class="col-12 col-md-12">
        ...
        <textarea class="form-control" placeholder="Enter description" name="description" rows="5">{{ old('description', $model->description) }}</textarea>
